                    ...
                    <label>
                        ...
                        <div style={{ marginTop: 8, padding: 8, backgroundColor: '#f0f0f0', borderRadius: 4, marginBottom: 8 }}>
                            <p style={{ margin: '0 0 8px 0', fontSize: 14 }}>
                                {selectionMode === null ? (
                                    <>
                                        {route.start ? <span>✓ Start: ({route.start.lat.toFixed(4)}, {route.start.lng.toFixed(4)}) </span> : <span>Click "Select Start" to begin</span>}
                                        {route.start && route.end && <span>✓ End: ({route.end.lat.toFixed(4)}, {route.end.lng.toFixed(4)})</span>}
                                    </>
                                ) : (
                                    <span>Click on the map to select {selectionMode === 'start' ? 'START' : 'END'} point</span>
                                )}
                            </p>
